        ...
        <div>
            ...
            @section('content')
                <style>
                    ...
                    =
                    "
                    {{url('https://cdnjs.cloudflare.com/ajax/libs/gijgo/1.9.13/combined/css/gijgo.min.css')}}
                    "
                    rel
                                    ...
                                    <div class="card-body">
                                        <div class="form-group{{ $errors->has('nama_aplikasi') ? ' has-error' : '' }}">
                                            ...
                                            <input type="text" class="form-control" name="nama_aplikasi"
                                                   value="{{old('nama_aplikasi')}}" placeholder="Nama Aplikasi">
                                        </div>
                                        <div class="form-group">
                                            <label for="ip_vps">Ip Vps</label>
                                            <input type="text" class="form-control" name="ip_vps"
                                                   value="{{old('id_vps')}}" placeholder="Ip Vps">
                                            @if ($errors->any())
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="ip_public"
                                                   value="{{old('ip_public')}}" placeholder="Ip Public">
                                            @if ($errors->any())
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select id="perangkat" class="form-control" name="id_perangkat">
                                                ...
                                                @foreach ($data_perangkat as $perangkat)
                                                    <option
                                                        value="{{ $perangkat->id_perangkat }}" {{ old('id_perangkat') == $perangkat->id_perangkat ? 'selected="selected"' : '' }}>
                                                        Nama Perangkat : {{ $perangkat->nama_perangkat }} || Ip Server
                                                        : {{ $perangkat->ip_server }}</option>
                                                @endforeach
                                            </select>
                                            @if ($errors->any())
...
@if ($errors->any())
                                    ...
                                    @endif
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select id="id_pengembang" class="form-control" name="id_pengembang">
                                                ...
                                                @foreach ($data_pengembang as $Pengembang)
                                                    <option
                                                        value="{{ $Pengembang->id_pengembang }}" {{ old('id_pengembang') == $Pengembang->id_pengembang ? 'selected="selected"' : '' }}>
                                                        Id Pengembang : {{ $Pengembang->id_pengembang }} || Nama
                                                        Pengembang 1 : {{ $Pengembang->nama_pengembang1 }} || Nama
                                                        Pengembang 2 : {{ $Pengembang->nama_pengembang1 }} || Nama
                                                        Pengembang 3: {{ $Pengembang->nama_pengembang1 }} </option>
                                                @endforeach
                                            </select>
                                            @if ($errors->any())
                                                {!! $errors->first('id_pengembang', '<p style="font-size: 12px; color:red">ERROR! input Id Pengembang Tidak Boleh Sama</p>') !!}
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="waktu_pengembangan" type="date" class="tm form-control"
                                                   id="datepicker" value="{{old('waktu_pengembangan')}}"
                                                   data-date-format="DD/MMM/YYYY" required autofocus>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select id="id_database" class="form-control" name="id_database">
                                                ...
                                                @foreach ($database as $base)
                                                    <option
                                                        value="{{ $base->id_database }}" {{ old('id_database') == $base->id_database ? 'selected="selected"' : '' }}> {{ $base->jenis_database }}</option>
                                                @endforeach
                                            </select>
                                            @if ($errors->any())
                                        ...
                                        <!--code Framework-->
                                        <div class="form-group">
                                            <label for="id_framework_platform">--Jenis Framework Yg Dipakai & Jenis
                                                Platform Yg diHasilkan--</label>
                                            <select id="id_framework_platform" class="form-control"
                                                    name="id_framework_platform">
                                                <option value=" ">--Pilih Jenis Framework Yg Dipakai & Jenis Platform Yg
                                                    diHasilkan--
                                                </option>
                                                @foreach ($frame as $work)
                                                    <option
                                                        value="{{ $work->id_framework_platform }}" {{ old('id_framework_platform') == $work->id_framework_platform ? 'selected="selected"' : '' }}> {{ $work->jenis_framework }}
                                                        || {{ $work->jenis_platform }}</option>
                                                @endforeach
                                            </select>
                                            @if ($errors->any())
