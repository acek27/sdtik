...
@section('content')
            ...
            <div class="col-sm-1">
                <td><select name="keterangan" class="form-control show-tick" id="list">
                        ...
                        @foreach($data_rak as $rak)
                            <option
                                value="{{ $rak->id_rak }}" {{ $pilihan == $rak->id_rak ? 'selected="selected"' : '' }}> {{ $rak->nomer_rak }}</option>
                        @endforeach
                    </select></td>
            </div>
        </div>
    ...
    <center>
        @if($pilihan==0)
            <a href="{{url('/seluruh_rak/cetak_pdf_seluruh')}}" class="btn btn-primary" target="_blank">CETAK PERANGKAT
                SELURUH</a>
        @else
            <a href="{{url('/cetak_rak/cetak_pdf_rak/{$pilihan}')}}" class="btn btn-primary" target="_blank">CETAK
                PERANGKAT PER RAK</a>
        @endif
    </center>
    <br>
        ...
        @push('script')
            <script>
                ...
                $('#list').change(function () {
                    document.location.href = '{{route('data_seluruh.index')}}/?id=' + $('#list').val();
                });
                $('body').on("click", '.show-data', function (e) {
                    $('#myModal').modal("show");
                    $.get("{{url('/anyData')}}/" + $(this).attr('data-id'), function (data) {
                        console.log(data);
                        ...
                        $('#print').attr("href", "{{url('/cetakaplikasi/cetak_pdf_aplikasi/')}}/" + data.id_aplikasi);
                    });
                });
                $('body').on("click", '.show-nama_aplikasi', function (e) {
                    ...
                    $('#myModal1').modal("show");
                    $('#myModal').modal("hide");
                    $.get("{{url('/anyDataaplikasi')}}/" + $(this).attr('data-id'), function (data) {
                        console.log(data);
                        ...
                        $('#print2').attr("href", "{{url('/cetakserver/cetak_pdf_server')}}/" + val1);
                    });
                });
            </script>
    @endpush
